    ...
    <div class="p-4">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
            <div class="flex flex-col sm:flex-row gap-2">
                ...
                <select wire:model="groupBy" class="select select-bordered w-full sm:w-auto">
                ...
                <div class="columns-2 sm:columns-3 md:columns-4 lg:columns-5 xl:columns-6 gap-4 space-y-4">
                    ...
                    <div class="break-inside-avoid mb-4 relative group">
                        ...
                        <div class="relative bg-base-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                            ...
                            <!-- Image -->
                            <img src="{{ $img->url }}" alt="{{ $img->caption ?? 'Gallery image' }}" class="w-full h-auto cursor-pointer hover:scale-105 transition-transform duration-300" wire:click="openModal({{ $img->id }})" loading="lazy">
            ...
            <div class="bg-base-100 rounded-lg max-w-4xl max-h-[90vh] overflow-hidden" @click.stop>
                <div class="flex flex-col md:flex-row">
                    ...
                    <div class="md:w-2/3 bg-black flex items-center justify-center">
                        <img src="{{ $modalImage->url }}" alt="{{ $modalImage->caption ?? 'Gallery image' }}" class="max-w-full max-h-[60vh] object-contain">
                    </div>
                    ...
                    <div class="md:w-1/3 p-6 space-y-4">
                        <div class="flex justify-between items-start">
                            ...
                            <button wire:click="closeModal" class="btn btn-ghost btn-sm">✕</button>
                        </div>

                        <div class="space-y-3">
                            @if($modalImage->caption)
                            <div>
                                <label class="text-sm font-medium text-gray-600">Caption</label>
                            ...
                            @if($modalImage->post)
                            <div>
                                <label class="text-sm font-medium text-gray-600">Post</label>
                            ...
                            <div>
                                ...
                                <p class="text-sm">{{ $modalImage->file_size ?? 'Unknown' }}</p>
                            </div>
                        </div>

                        <!-- Actions -->
                        <div class="flex gap-2 pt-4">
                            <button wire:click="toggleImageSelection({{ $modalImage->id }})" class="btn btn-sm {{ in_array($modalImage->id, $selectedImages) ? 'btn-error' : 'btn-primary' }}">
                                {{ in_array($modalImage->id, $selectedImages) ? 'Deselect' : 'Select' }}
                            </button>
                            <a href="{{ $modalImage->url }}" target="_blank" class="btn btn-sm btn-ghost">
                                View Full Size
                            </a>
                        </div>
                    </div>
