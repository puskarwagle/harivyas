{{-- resources/views/livewire/faq-front.blade.php --}}
<div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <div class="tabs tabs-boxed justify-center mb-8 flex-wrap">
        @foreach($categories as $category)
            <button class="tab {{ $selectedCategory === $category['slug'] ? 'tab-active' : '' }}"
                        ...
                        @if(!empty($faq['tags']))
                            <div class="mt-4 flex flex-wrap gap-2">
                                @foreach($faq['tags'] as $tag)
                                    <div class="badge badge-base-content badge-sm">
                                        {{ $tag['name'] }}
                                    </div>
                                @endforeach
                            </div>
    ...
    {{-- No Results Message --}}
    @if(count($filteredFaqs) === 0 && count($faqs) > 0)
