    ...
    <div x-data="digitalMedia()" class="min-h-screen">
        ...
        <div class="hero py-16">
            <div class="hero-content text-center">
                <div class="max-w-2xl">
                    <h1 class="text-5xl font-bold text-primary mb-4">डिजिटल मीडिया</h1>
                    <h2 class="text-2xl font-semibold text-primary mb-2">Digital Media</h2>
                    <p class="text-base-content">Sacred texts, devotional music, and spiritual teachings in digital format</p>
                </div>
            </div>
        </div>
        <!-- Search and Filter -->
        <div class="container mx-auto px-4 py-8">
            <div class="flex flex-col sm:flex-row gap-4 mb-8">
                <div class="flex-1">
                    <div class="form-control">
                        <div class="input-group">
                            <input type="text" x-model="searchQuery" @input="filterContent" placeholder="Search media..." class="input input-bordered flex-1 focus:input-primary">
                ...
                <div class="form-control">
                    <select x-model="selectedCategory" @change="filterContent" class="select select-bordered focus:select-primary">
            ...
            <div class="tabs tabs-boxed mb-8 flex-wrap justify-center">
                <template x-for="tab in tabs" :key="tab.id">
                    <a class="tab" :class="{'tab-active': activeTab === tab.id}" @click="activeTab = tab.id; filterContent()" x-text="tab.name"></a>
                </template>
            </div>
                    ...
                    <div class="card bg-base-100 shadow-xl border border-primary hover:shadow-2xl transition-shadow">
                        <figure class="relative">
                            ...
                            <div class="absolute bottom-4 right-4" x-show="item.duration">
                                <div class="badge badge-neutral badge-outline bg-base-100 text-base-content border-base-200">
                        ...
                        <div class="card-body">
                            <h3 class="card-title text-primary" x-text="item.title"></h3>
                            <p class="text-base-content text-sm mb-2" x-text="item.description"></p>
                            ...
                            <div class="text-xs text-base-content mb-4">
                            ...
                            <div class="card-actions justify-end flex-wrap">
                                <!-- Play/View Button -->
                                <button x-show="item.type === 'audio' || item.type === 'video'" @click="playMedia(item)" class="btn btn-primary btn-sm">
                                    <i :class="item.type === 'audio' ? 'fas fa-play' : 'fas fa-play-circle'" class="mr-1"></i>
                                    <span x-text="item.type === 'audio' ? 'Play' : 'Watch'"></span>
                                </button>

                                <button x-show="item.type === 'ebook' || item.type === 'wallpaper'" @click="viewItem(item)" class="btn btn-primary btn-sm">
                                    <i class="fas fa-eye mr-1"></i>
                                    View
                                </button>

                                <!-- Download Button -->
                                <button @click="downloadItem(item)" class="btn btn-outline btn-sm">
                                    <i class="fas fa-download mr-1"></i>
                                    Download
                                </button>

                                <!-- Share Button -->
                                <button @click="shareItem(item)" class="btn btn-ghost btn-sm">
            ...
            <div x-show="filteredContent.length === 0" class="text-center py-16">
                <i class="fas fa-search text-6xl text-base-200 mb-4"></i>
                <h3 class="text-2xl font-bold text-base-content mb-2">No media found</h3>
                <p class="text-base-content">Try adjusting your search or filter criteria</p>
            </div>
        </div>
        ...
        <div x-show="showPlayer" class="modal modal-open">
            <div class="modal-box max-w-4xl">
                ...
                <div x-show="currentMedia?.type === 'video'" class="mb-4">
                    <video x-ref="videoPlayer" controls class="w-full rounded-lg" :poster="currentMedia?.thumbnail">
                ...
                <div x-show="currentMedia?.type === 'audio'" class="mb-4">
                    <div class="bg-gradient-to-r from-base-100 to-primary rounded-lg p-6">
                        <div class="flex items-center mb-4">
                            <img :src="currentMedia?.thumbnail" :alt="currentMedia?.title" class="w-20 h-20 rounded-lg object-cover mr-4">
                            <div>
                                <h4 class="font-semibold text-primary" x-text="currentMedia?.title"></h4>
                                <p class="text-sm text-base-content" x-text="currentMedia?.description"></p>
                            </div>
                        </div>
